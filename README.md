### Sono Alessandro 👋

Studio **Computer Science (Enterprise Applications)** all'UNICAL.

**Cosa faccio davvero:**
* 🚀 **Shipping Real Software:** Ho sviluppato e portato in produzione da zero un gestionale aziendale completo per un cliente B2B (servizio NCC).
* 💻 **Full-Stack / Enterprise:** Progetto e sviluppo web app end-to-end, gestendo sia l'architettura backend che l'interfaccia frontend.

**Il mio Tech Stack:**
* ⚙️ **Backend & Architettura:** Java (Spring Boot, Maven, JPA/Hibernate), Node.js (Express), Python, C++
* 🖥️ **Frontend:** JavaScript/TypeScript, React, Angular, HTML5/CSS3
* 🗄️ **Database & Tooling:** MySQL, Docker, Git, Postman

**Progetti in Evidenza:**
* 🚗 **[gestionale-ncc-locridea](#):** Gestionale B2B in produzione per azienda NCC.
* 🏠 **[webapp-immobiliarende](#):** Web application full-stack sviluppata interamente da zero (Solo project).
* 🤖 **[BuroBot](#):** Il mio assistente burocratico per analizzare documenti. Sviluppato per l'AI Week di Develhope.

<br>
